...
public class RemoteCommands extends CommonCommands {
    ...
    void findFile(String filename,
            @ShellOption(value = { "--path",
                    "-p" }, valueProvider = RemoteFileCompleter.class, defaultValue = "/") String path) {
    ...
    @ShellMethod(key = "reboot", value = "reboot")
    @ShellMethodAvailability("availabilityCheck")
    void reboot() {
        scmd("reboot");
    }
    ...
    void mem_stat(
        ...
        if (memMonitorRunning) {
            System.out.println(
                    ColorPrinter.yellow("Memory monitoring already running for service: " + memMonitoringService));
        ...
        try {
            ...
            if (exename != null) {
                ...
                try {
                    ...
                    System.out.println(
                            ColorPrinter.yellow("Warning: Could not deploy get_process_name.sh: " + e.getMessage()));
                }
                ...
                if (processname.isEmpty()) {
                    System.out.println(ColorPrinter
                            .red("Error: Could not determine process name for service '" + servicename + "'"));
                    return;
                }
            }
    ...
    void monitor_start(
            @ShellOption(value = { "--ser",
                    "-s" }, valueProvider = ServiceCommands.ServiceProvider.class, defaultValue = "") String s,
        ...
        try {
            ...
            // Start monitoring in background - pass sanitized name for PID file and
            // get_process_name script path
            String startCmd = String.format(
                    "nohup %s '%s' '%s' %d '%s' '%s' > /dev/null 2>&1 &",
                    remoteMonitorScriptPath, s, monitorLogFile, interval, monitorPidFile,
                    remoteGetProcessNameScriptPath);
            ...
            if (verifyResult.contains("RUNNING")) {
                // Also verify log file is being created and check if process is actually
                // running
                ...
                runCommandShort(logCheckStream, Util.fullMachineName(machine), userName, passwd,
                        "test -f " + monitorLogFile + " && (wc -l " + monitorLogFile + "; tail -2 " + monitorLogFile
                                + ") || echo '0'");
                ...
                if (lineCount < 2) {
                    System.out.println(
                            ColorPrinter.yellow("⚠ Monitoring process started but log file not yet populated"));
                ...
                if (processNotRunning) {
                    System.out.println(
                            ColorPrinter.yellow("  ⚠ Warning: Target service '" + s + "' is not currently running"));
                    System.out.println(ColorPrinter
                            .yellow("     Monitoring will show 'stopped' or 'not_running' until the service starts"));
                    System.out.println(ColorPrinter.yellow(
                            "     The monitor script will continue running and capture data when service starts"));
                }
                ...
                if (monitorLogFile != null) {
                    ...
                    runCommandShort(csvCheckStream, Util.fullMachineName(machine), userName, passwd,
                            "test -f " + monitorLogFile + " && (wc -l " + monitorLogFile + "; tail -3 " + monitorLogFile
                                    + ") || echo 'No CSV file'");
    ...
    void monitor_cleanup() {
        try {
            ...
            if (remaining > 0) {
                System.out.println(ColorPrinter
                        .yellow("⚠ Warning: " + remaining + " process(es) still running (may require manual cleanup)"));
            } else {
    ...
    void monitor_status() {
        ...
        try {
            ...
            runCommandShort(checkStream, Util.fullMachineName(machine), userName, passwd,
                    "test -f " + monitorPidFile + " && ps -p $(cat " + monitorPidFile
        ...
        try {
            ...
            for (int i = 1; i < lines.length; i++) {
                ...
                if (parts.length >= 9) {
                    try {
                        // New format:
                        // timestamp,process_name,pid,full_command_path,threads,mem_percent,mem_vsz_kb,unknown_field,cpu_percent
                        if (i == 1) {
    ...
    private void displayMemoryUsageGraph(int maxItems) throws IOException {
        ...
        runCommandShort(outputStream, Util.fullMachineName(machine), userName, passwd,
                "top -bn1 | grep -v 'top -' | grep -v 'Mem:' | grep -v 'CPU:' | grep -v 'Load' | grep -E '^\\s*[0-9]+' | head -n "
                        + maxItems);
    ...
    private void displayCpuUsageGraph(int maxItems) throws IOException {
        ...
        if (!processes.isEmpty()) {
            ...
            for (ProcessCpuInfo proc : processes) {
                ...
                String name = proc.name.length() > maxNameLength ? proc.name.substring(0, maxNameLength - 3) + "..."
                        : proc.name;
    ...
    private void displayDiskUsageGraph(int maxItems) throws IOException {
        ...
        if (!binaries.isEmpty()) {
            ...
            for (BinaryDiskInfo binary : binaries) {
                String name = binary.name.length() > maxNameLength ? binary.name.substring(0, maxNameLength - 3) + "..."
                        : binary.name;
