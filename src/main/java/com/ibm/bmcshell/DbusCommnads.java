...
public class DbusCommnads extends CommonCommands{
    ...
    public void introspect(@ShellOption(value = {"--ser", "-s"},defaultValue="") String service,@ShellOption(value = {"--path", "-p"},defaultValue="")String path){
        if(service.equals("") ){
        ...
        currentPath=path;
        runCommand(Utils.fullMachineName(machine),userName,passwd,String.format("busctl introspect %s %s",service,path));

    }
    @ShellMethod(key="bs.search",value = "eg: bs.search network")
    @ShellMethodAvailability("availabilityCheck")
    public void search(String service){
        runCommand(Utils.fullMachineName(machine),userName,passwd,String.format("busctl list | grep %s",service));
    ...
    public void tree(String service){
        ...
        runCommand(Utils.fullMachineName(machine),userName,passwd,String.format("busctl tree %s",service.equals("*")?"":service));
    }
    ...
    public void managed_objects(String service,String path){
        runCommand(Utils.fullMachineName(machine),userName,passwd,String.format("busctl call %s %s org.freedesktop.DBus.ObjectManager GetManagedObjects --verbose",service,path));
    }
    ...
    @ShellMethodAvailability("availabilityCheck")
    public void     call(@ShellOption(value = {"--ser", "-s"},defaultValue="")  String service,@ShellOption(value = {"--path", "-p"},defaultValue="") String path,@ShellOption(value = {"--iface", "-i"},defaultValue="") String iFace,@ShellOption(value = {"--method", "-m"}) String method,@ShellOption(value = {"--sig", "-s"},defaultValue="") String sig, @ShellOption(value = {"--args", "-a"},defaultValue="") String args){
        if(service.equals("") ){
            service=currentService;
            System.out.println("Service is null, using previous service "+service);
        }
    ...
    @ShellMethodAvailability("availabilityCheck")
    public void call(String iFace,String method,@ShellOption(value = {"--sig", "-s"},defaultValue="") String sig, @ShellOption(value = {"--args", "-a"},defaultValue="") String args){
    ...
    public void setProperty(@ShellOption(value = {"--ser", "-s"},defaultValue="")  String service,
        ...
        currentIface=iFace;
        String commd=String.format("busctl set-property %s %s %s %s %s %s",service,path,iFace,property,sig,args);
        commd = commd + " --verbose";
    ...
    public void property(@ShellOption(value = {"--ser", "-s"},defaultValue="") String service,
        ...
        runCommand(Utils.fullMachineName(machine),userName,passwd,String.format("busctl get-property %s %s %s %s %s",service,path,iface,prop,"--verbos"));
